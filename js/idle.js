...
var isVerified = "No";
  ...
  firebase.auth().onAuthStateChanged(function(user) {
    if (user) {
      ...
      var presenceRef = firebase.database().ref(`CHATROOM/ONLINEUSERS/${firebase.auth().currentUser.uid}`);
     ...
     presenceRef.set({ParentID:firebase.auth().currentUser.uid+"ParentStatus",UserID:firebase.auth().currentUser.uid+"Status",Name:firebase.auth().currentUser.displayName,Photo:firebase.auth().currentUser.photoURL,status:"Active"});


      firebase.database().ref(`CHATROOM/USERDETAILS/${userid}`).once('value',(data)=>{

        firebase.database().ref(`CHATROOM/BLOCKEDIPS`).on('value',(blocklist)=>{
        ...
        if(data.hasChild("Phone")  && data.hasChild("FBLink") && data.hasChild("NickName") && data.hasChild("FullName") && blocklist.hasChild(firebase.auth().currentUser.uid) === false){



          //console.log("user is elegible to show online")
  ...
  setTimeout(function(){
    $(function() {
        ...
        var awayCallback = function() {
            presenceRef.set({ParentID:firebase.auth().currentUser.uid+"ParentStatus",UserID:firebase.auth().currentUser.uid+"Status",Name:firebase.auth().currentUser.displayName,Photo:firebase.auth().currentUser.photoURL,status:"Away"});
        };
        var awayBackCallback = function() {
            presenceRef.set({ParentID:firebase.auth().currentUser.uid+"ParentStatus",UserID:firebase.auth().currentUser.uid+"Status",Name:firebase.auth().currentUser.displayName,Photo:firebase.auth().currentUser.photoURL,status:"Active"});
        };
        var hiddenCallback = function() {
            presenceRef.set({ParentID:firebase.auth().currentUser.uid+"ParentStatus",UserID:firebase.auth().currentUser.uid+"Status",Name:firebase.auth().currentUser.displayName,Photo:firebase.auth().currentUser.photoURL,status:"Not Viewing"});
        };
        ...
        firebase.database().ref(`CHATROOM/ONLINEUSERS/${firebase.auth().currentUser.uid}`).remove()
        //console.log("user isnt eligible not showing online to other users.")})
      }
